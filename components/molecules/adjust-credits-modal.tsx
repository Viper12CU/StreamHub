...
import { adjustCredits, clearCreditsCache } from "@/lib/api/credits"
...
export function AdjustCreditsModal({ customerId, userName, onClose, onCreated }: AdjustCreditsModalProps) {
  ...
  const canProceed = step === 0 ? Number(balance) >= 0 : true

  const handleSubmit = async () => {
    try {
      setLoading(true)
      setError(null)
      await adjustCredits(customerId, { balance: Number(balance), reason: reason || undefined })
      clearCreditsCache()
      onCreated()
    } catch (err) {
    ...
    <div className="fixed inset-0 flex items-center justify-center" style={{ zIndex: 9999 }}>
      ...
      <div className="relative glass rounded-2xl w-[520px] max-h-[85vh] overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/5">
          <div>
            <h2 className="text-lg font-bold text-on-surface">Ajustar Créditos</h2>
            <p className="text-xs text-on-surface-variant mt-1">
              {userName ? `Para: ${userName}` : "Selecciona un usuario"}
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          {step === 0 ? (
            <div className="space-y-4">
              <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-center gap-2">
                <Icon name="information" className="text-amber-500 text-sm shrink-0" />
                <p className="text-xs text-amber-500">
                  Este acción establecerá el balance del usuario al valor exacto ingresado, sobrescribiendo el balance actual.
                </p>
              </div>
              <div className="space-y-2">
                <label htmlFor="adjust-balance" className="text-[11px] font-semibold text-on-surface-variant uppercase tracking-wider">
                  Nuevo Balance
                </label>
              ...
              <div className="space-y-2">
                <label htmlFor="adjust-reason" className="text-[11px] font-semibold text-on-surface-variant uppercase tracking-wider">
                  Razón (Opcional)
                </label>
                <textarea
                  id="adjust-reason"
                  ...
                  placeholder="Motivo del ajuste de créditos..."
                  maxLength={500}
                  rows={3}
            ...
            <div className="space-y-4">
              <div className="glass rounded-xl p-5 border border-white/5 space-y-3">
                ...
                <div className="space-y-2 text-xs">
                  ...
                  <div className="flex justify-between">
                    <span className="text-on-surface-variant">Nuevo Balance</span>
                    <span className="text-primary font-bold">${Number(balance).toFixed(2)}</span>
                  </div>
                  {reason && (
                    <div className="flex justify-between">
        ...
        <div className="p-6 border-t border-white/5 flex gap-2">
          ...
          {step === 0 ? (
            ...
            <button
              ...
              className="flex-1 px-4 py-2.5 bg-primary text-white text-xs font-semibold rounded-xl hover:bg-primary/90 transition-colors disabled:opacity-40 disabled:cursor-not-allowed shadow-lg shadow-primary/20"
            >
              {loading ? "Procesando..." : "Ajustar Balance"}
            </button>
          )}
        </div>
      </div>
