    ...
    <div className="fixed inset-0 flex items-center justify-center p-4" style={{ zIndex: 9999 }}>
      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] bg-surface-container-lowest border border-white/10 rounded-2xl overflow-hidden shadow-2xl shadow-black/50" onClick={(e) => e.stopPropagation()}>
          ...
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex-1 flex items-center gap-2">
              <div className={cn(
                ...
                s < step ? "bg-primary text-on-primary" :
                s === step ? "bg-primary text-on-primary ring-4 ring-primary/20" :
        ...
        <div className="flex items-center justify-between p-5 border-t border-white/5 bg-surface-container-lowest/50">
          ...
          {step < 3 ? (
            <button onClick={() => setStep(step + 1)} className="flex items-center gap-2 px-5 py-2.5 bg-primary text-on-primary text-xs font-semibold rounded-xl hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20">
              Siguiente
