...
export function GrantCreditsModal({ customerId, userName, onClose, onCreated }: GrantCreditsModalProps) {
    ...
    <div className="fixed inset-0 flex items-center justify-center" style={{ zIndex: 9999 }}>
      ...
      <div className="relative glass rounded-2xl w-[520px] max-h-[85vh] overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/5">
          <div>
            <h2 className="text-lg font-bold text-on-surface">Otorgar Créditos</h2>
            <p className="text-xs text-on-surface-variant mt-1">
              {userName ? `A: ${userName}` : "Selecciona un usuario"}
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          {step === 0 ? (
            <div className="space-y-4">
              <div className="space-y-2">
                <label htmlFor="grant-amount" className="text-[11px] font-semibold text-on-surface-variant uppercase tracking-wider">
                  Monto a Otorgar
                </label>
              ...
              <div className="space-y-2">
                <label htmlFor="grant-reason" className="text-[11px] font-semibold text-on-surface-variant uppercase tracking-wider">
                  Razón (Opcional)
                </label>
                <textarea
                  id="grant-reason"
                  ...
                  placeholder="Motivo del otorgamiento de créditos..."
                  maxLength={500}
                  rows={3}
        ...
        <div className="p-6 border-t border-white/5 flex gap-2">
          ...
          {step === 0 ? (
            ...
            <button
              ...
              disabled={loading}
              className="flex-1 px-4 py-2.5 bg-green-500 text-white text-xs font-semibold rounded-xl hover:bg-green-500/90 transition-colors disabled:opacity-40 disabled:cursor-not-allowed shadow-lg shadow-green-500/20"
            >
              {loading ? "Procesando..." : "Otorgar Créditos"}
            </button>
          )}
        </div>
      </div>
